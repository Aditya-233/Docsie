...
export default function AuthModal({ isOpen, isMandatory = false, onClose, onAuthSuccess }: AuthModalProps) {
  const [tab, setTab] = useState<'guest' | 'login' | 'signup'>('guest');
  const [guestName, setGuestName] = useState('Aditya');
  ...
  const handleGuestSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const finalName = guestName.trim() || 'Guest Collaborator';
    setError('');
    const user = authManager.loginAsGuest(finalName, selectedColor);
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#282a2c] rounded-2xl shadow-2xl border border-[#dadce0] dark:border-[#444746] w-full max-w-md overflow-hidden text-[#202124] dark:text-[#e3e3e3] text-xs"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-[#f1f3f4] dark:border-[#3c4043]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-semibold text-gray-900 dark:text-white">
                {isMandatory ? 'Sign in to Docsie' : 'Account & Identity'}
              </h2>
              <p className="text-[11px] text-gray-500 dark:text-gray-400">
                {isMandatory ? 'Choose your collaborator profile to proceed' : 'Choose how you want to collaborate'}
              </p>
            </div>
          </div>
          {!isMandatory && (
        ...
        {/* Error Alert */}
        {error && (
          <div className="mx-5 sm:mx-6 mt-4 p-2.5 rounded-lg bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-xs font-medium">
            {error}
          </div>
        )}
            ...
            <form onSubmit={handleGuestSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-gray-500 mt-1">
                  This name will appear on your remote cursor, caret tag, and comments across all devices.
                </p>
              </div>
              ...
              >
                <span>Continue & Start Collaborating</span>
          ...
          {tab === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl shadow-md transition-all flex items-center justify-center gap-2 text-xs cursor-pointer"
              >
